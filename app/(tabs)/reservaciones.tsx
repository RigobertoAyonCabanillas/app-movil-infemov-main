...
import { obtenerClasesGimnasio } from '@/services/api';
...
// Mapa exacto basado en tus capturas de IDs
...
export default function ReservacionesScreen() {
  ...
  // FILTRO DINÁMICO: Solo muestra disciplinas que existen en las clases cargadas
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      const clasesMapeadas: Clase[] = data.map((item: any) => ({
        id: item.id?.toString(),
        nombre: MAPA_DISCIPLINAS[item.tipoClase_ID] || 'Clase', 
        horaInicio: item.horaIncio || '00:00', 
        horaFin: item.horaFin || '00:00',
        coach: item.asistenciaCoach || 'Staff',
        dia: item.fecha.split('T')[0], 
            ...
            <ScrollView contentContainerStyle={styles.chipContainer}>
              {disciplinasParaFiltro.map((item) => (
                <Chip
                  ...
                  onPress={() => {
                    setDisciplinaSeleccionada(item);
            ...
            {diasSemana.map((dia) => {
              ...
              return (
                <List.Accordion
                  ...
                  description={`${clasesDelDia.length} disponibles`}
                  ...
                  {clasesDelDia.length > 0 ? (
                    clasesDelDia.map((clase) => (
                      <List.Item
                        ...
                        left={props => <List.Icon {...props} icon="clock-outline" color={COLORS.accent} />}
                      />
                    ))
                  ) : (
                    <View style={styles.emptyContainer}>
                      <Text style={styles.emptyText}>
                        No hay clases de {disciplinaSeleccionada === 'Todas las Clases' ? 'nada' : disciplinaSeleccionada} este día.
                      </Text>
                    </View>
                  )}
                </List.Accordion>
